...
import com.dep.sula.dto.OrderDetailDTO;
import com.dep.sula.entity.*;
...
import com.dep.sula.util.PlaceOrderTM;
...
public class PlaceOrderViewController implements Initializable {
    ...
    void btnOrder(ActionEvent event) throws Exception {
        ...
        int i=orderBO.placeOrder(orderDTO);
        if(i>0) {
            connection.commit();
    ...
    private void loadAllitems() throws SQLException {
       ArrayList<ItemDTO> items= itemBO.findAllItems();
        for (ItemDTO itemDTO:items) {
            tempItems.add(new Item(itemDTO.getItemCode(),itemDTO.getDescription(),itemDTO.getQty(),itemDTO.getUprice()));
        }
